...
function AppointmentRow({
  ...
  const disableLateNoShowTitle =
    "Solo se puede marcar cuando la cita ya ha empezado (después de la hora de inicio).";

  // Mostrar Undo solo si tiene sentido:
  // - status late o no_show
  // - y no está cobrado (charged)
  const canUndo =
    ...
    <tr style={{ borderBottom: "1px solid #222" }}>
      ...
      <td style={{ padding: 10, whiteSpace: "nowrap" }}>
        ...
        <button
          ...
          disabled={future}
          title={future ? disableLateNoShowTitle : undefined}
          style={{
            marginRight: 10,
